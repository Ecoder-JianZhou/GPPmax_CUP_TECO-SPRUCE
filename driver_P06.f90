        ...
        do iclim = 1, nforcing 
            ...
            if(do_annual_mcmc) then
                ...
                if (iyear < 2014) then
                    ...
                    ! tree
                    if(iyear .eq. 2021)then
                    st%sp(1)%s_vea  = st%sp(1)%s_vea_1*1.6
                    ! st%sp(i)%s_ved = st%sp(i)%s_ved_1
                    st%sp(1)%s_jea  = st%sp(1)%s_vea_1*1.6
                    ! st%sp(i)%s_jed = st%sp(i)%s_ved_1
                    st%sp(1)%Entrpy = st%sp(1)%Entrpy_1
                    st%sp(1)%Vcmax0 = st%sp(1)%Vcmax0_1*1.2
                    else
                    ...
                    elseif (iyear .eq. 2020 .and. iday .eq. 1 .and. ihour .eq. 0) then
                        ...
                        do i = 2,3
                            if(i .eq. 2)then
                            st%sp(i)%s_vea = st%sp(i)%s_vea_6*1.7 ! def_s_vea(i)    !
                            ! st%sp(i)%s_ved = st%sp(i)%s_ved_7
                            st%sp(i)%s_jea = st%sp(i)%s_vea_6*1.7 ! def_s_vea(i) !
                            ! st%sp(i)%s_jed = st%sp(i)%s_ved_7
                            st%sp(i)%Entrpy = st%sp(i)%Entrpy_6 ! def_Entrpy(i) !
                            st%sp(i)%Vcmax0 = st%sp(i)%Vcmax0_6*1.3 ! def_Vcmax0(i) !
                            else
            ...
            ! if ((iday .eq. 1) .and. (ihour .eq. 0)) print*, st%sp(1)%Vcmax0
